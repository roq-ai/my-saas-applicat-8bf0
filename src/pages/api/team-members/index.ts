...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createTeamMember() {
    ...
    const body = { ...req.body };
    if (body?.pose_estimation_result?.length > 0) {
      const create_pose_estimation_result = body.pose_estimation_result;
      body.pose_estimation_result = {
        create: create_pose_estimation_result,
      };
    } else {
      delete body.pose_estimation_result;
    }
